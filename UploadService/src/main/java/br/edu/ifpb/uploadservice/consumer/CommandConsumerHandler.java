...
import br.edu.ifpb.uploadservice.consumer.command.EfetuarReservaCommand;
...
public class CommandConsumerHandler {
    ...
    public Message efetuaReservaEspaco(CommandMessage<EfetuarReservaCommand> cm, PathVariables pv) {
        try {
            ...
            cm.getCommand().setStatus("RESERVA_REALIZADA");
        } catch (NenhumaUnidadeComEspacoDisponivelException e) {
            cm.getCommand().setStatus("RESERVA_REJEITADA");
            return withFailure();
        }
        return withSuccess();
    }
